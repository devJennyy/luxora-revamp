      ...
      <div className="flex flex-col text-start gap-10 !mt-5">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex xl:gap-3 gap-2 text-sm">
            {sizes?.map((size, index) => {
              return (
                <button key={index} className="flex justify-center items-center border w-11 h-11 cursor-pointer rounded-md hover:bg-primary hover:text-white active:bg-primary transition-default">
                  {size}
                </button>
              );
            })}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col items-start gap-3 text-secondary text-sm">
            {prices?.map((price, index) => {
              return <button key={index} className="hover:text-primary cursor-pointer">{price}</button>;
            })}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col items-start gap-3 text-secondary text-sm">
            {collections?.map((collection, index) => {
              return (
                <button key={index} className="hover:text-primary cursor-pointer">{collection}</button>
              );
            })}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start flex-wrap gap-3 text-secondary text-sm">
            {tags?.map((tag, index) => {
              return <button key={index} className="cursor-pointer">{tag}</button>;
            })}
          </div>
        </div>
      </div>
